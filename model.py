...
class SPCT(nn.Module):
    """
    Implementation ov Simple PCT, without neighbour embedding and with Offset Self-Attention
...
class PCT(nn.Module):
    """
    Implementation ov PCT, with neighbour embedding and with Offset Self-Attention
    ...
    def __init__(self, output_channels=10):
        ...
        self.linear1 = nn.Linear(1024, 512, bias=False)
        self.bn6 = nn.BatchNorm1d(512)
        self.dp1 = nn.Dropout(p=0.5) #TODO self.dp1 = nn.Dropout(p=args.dropout)
        self.linear2 = nn.Linear(512, 256)
        self.bn7 = nn.BatchNorm1d(256)
        self.dp2 = nn.Dropout(p=0.5) #nn.Dropout(p=args.dropout)
        self.linear3 = nn.Linear(256, output_channels)
...
class SA_Layer(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.q_conv = nn.Conv1d(channels, channels // 4, 1, bias=False) # Q
        self.k_conv = nn.Conv1d(channels, channels // 4, 1, bias=False) # K, queries and keys of same dimentionality
    ...
    def forward(self, x):
        ...
        # b, c, n
        x_r = torch.bmm(x_v, attention) #The self-attention output features F_sa are the weighted
